...
public class ApplicationController {
	...
	public ResponseEntity<?> applyJob(@PathVariable @NonNull Long jobId,
	...
	@PreAuthorize("hasRole('JOBSEEKER')")
	@SecurityRequirement(name = "bearerAuth")
	public List<MyApplicationResponseDto> getMyApplications(@AuthenticationPrincipal UserDetails userDetails) {
...
// | `DELETE` | `/applications/{id}` | Withdraw own application | Jobseeker
// (owner) |
// | `GET` | `/jobseekers/me/applications` | List own applications with current
// status | Jobseeker |
// | `GET` | `/jobs/{jobId}/applications` | List all applicants for a job |
// Employer |
// | `GET` | `/applications/{id}` | Get full application details | Owner |
// | `PATCH` | `/applications/{id}/status` | Advance/decline application status
// | Employer |
// | `PUT` | `/applications/{id}/notes` | Add/update private notes on an
